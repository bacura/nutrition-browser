#! /usr/bin/ruby
#encoding: utf-8
#Nutrition browser print page selector 0.0.7 (2026/01/19)
...
def language_pack( language )
	...
	l['ja'] = {
		...
		eiyo:		"栄養レシピ",
		full:		"フルレシピ",
		cb_plus:	"<img src='bootstrap-dist/icons/clipboard-plus.svg' style='height:2em; width:2em;'>",
...
res = db.query( "SELECT * FROM #{$TB_RECIPE} WHERE code=?", false, [code] )&.first
unless res
	puts "#{l[:non]}(#{code})"
	exit( 9 )
end
recipe_name = res['name']
recipe_dish = res['dish']
...
#### Setting palette
palette_name = []
r = db.query( "SELECT * from #{$TB_PALETTE} WHERE user=?", false, [user.name] )
...
<div class='container-fluid'>
	...
	<hr>
	<div align="center"><button class='btn btn-sm' id="cp2cb">#{l[:cb_plus]}</button><span id="recipe_print_url">#{$MYURL}printv.cgi?c=#{code}&t=1</span></div>
</div>
...
var openPrint = ( uname, code, template, dish ) => {
