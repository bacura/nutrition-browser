...
#encoding: utf-8
module DetectiveMod_00_gori
  def self.lp( language )
    l = { 'ja' => { mod_name: "ゴリ押し（最大5食品・順番制限なし）" }}
    return l[language]
  end
  ...
  def self.search( foods, foods_size, std_target, sw, depth, current_weights, best )
    if depth == foods_size
      return if ( current_weights.sum - 100 ).abs > 0.9  # 許容誤差
    ...
    loop do
      ...
      search( foods, foods_size, std_target, sw, depth + 1, current_weights + [w], best )
      w += 1
    end
  end
  ...
  def self.detective_module( foods, std_target, cfg )
    ...
    search( foods, foods.size, std_target, sw, 0, [], best )
